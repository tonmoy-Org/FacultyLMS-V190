        ...
        <div class="row recent-videos recent-video-slider slider-primary justify-content-center" data-aos="fade-up"
            ...
            @foreach(getArrayValue('links',$section->contents,[]) as $link)
                <div class="col-lg-12 text-center px-0">
                    @include('frontend.components.video',['source' => $link['video_source'], 'video' => $link['video'], 'class' => 'recent-video-player w-100', 'image' => getArrayValue('image', $link) , 'size' => '1030x520'])
                </div>
            @endforeach
        </div>
    </div>
